...
const artifactDir = path.join(root, "dist", "employeeportal");
...
if (packagedEnvironment.PORT !== "3081") {
...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    try {
      pageResponse = await fetch("http://127.0.0.1:3081/employee-portal/login");
      break;
    } catch {
      await delay(500);
    }
  ...
  if (!pageResponse?.ok) {
    throw new Error(`Packaged server did not return HTTP 200 for /employee-portal/login.\n${output}`);
  }
  ...
  const runtimeResponse = await fetch("http://127.0.0.1:3081/runtime-config.js");
  ...
  console.log("employeeportal production artifact smoke test passed:");
  console.log("- node server.js listened on port 3081");
  console.log("- GET /employee-portal/login returned HTTP 200");
